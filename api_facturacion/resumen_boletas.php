<?php
	...
	if (isset($data['id_documento'])) {
		$ids = $data['id_documento'];
	}
	if (isset($data['new_id_document'])) {
		$ids = $data['new_id_document'];
	}

	$array_cabecera = get_array_cabecera($data, $array_emisor);
	$procesarcomprobante = new Procesarcomprobante("Resumen de boletas", $rutas);
	$resp = $procesarcomprobante->procesar_resumen_boletas($array_cabecera, $array_detalle, $rutas, $ids, isset($data['app_id'])?$data['app_id']:null);
			...
			default:
				...
				//   die("ERROR: Unable to connect: " . $conn->connect_error);
					...
					if (isset($resp['cod_sunat'])) {
						$idsdOther = $resp['cod_sunat'];
					}
					$ticket = null;
					if (isset($resp['ticket'])) {
						$respTicket = $resp['ticket'];
						if (isset($respTicket['cod_ticket'])) {
							$ticket = $respTicket['cod_ticket'];
						}
						if (isset($respTicket['cod_sunat'])) {
							$idsdOther = $respTicket['cod_sunat'];
						}
					}
					if (is_array($idDocumento)) {
						$string = "UPDATE sal_sale_documents SET idsd_other = " . $idsdOther . ", summary_ticket = " . $ticket . ", sunat_log = '". json_encode($resp) ."' , url_cdr = '". $resp['ruta_cdr'] ."' WHERE id in (" . implode(',', $idDocumento) . ");";
					} else {
						$string = "UPDATE sal_sale_documents SET idsd_other = " . $idsdOther . ", summary_ticket = " . $ticket . ", sunat_log = '". json_encode($resp) ."' , url_cdr = '". $resp['ruta_cdr'] ."' WHERE id = " . $idDocumento . ";";
					}
					$result = $conn->query($string);
					// echo "Number of rows: $result->num_rows";
					// $result->close();
