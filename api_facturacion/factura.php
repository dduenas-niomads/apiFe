<?php
	...
	$resp['ruta_pdf'] = '/controllers/prueba.php?tipo=factura&id=' . $nombre_archivo;
			...
			default:
				...
				//   die("ERROR: Unable to connect: " . $conn->connect_error);
					...
					if (is_array($idDocumento)) {
						$string = "UPDATE sal_sale_documents SET idsd_other = " . $resp['cod_sunat'] . ", sunat_log = '". json_encode($resp) ."' , url_cdr = '". $resp['ruta_cdr'] ."' WHERE id in (" . implode(',', $idDocumento) . ");";
					} else {
						$string = "UPDATE sal_sale_documents SET idsd_other = " . $resp['cod_sunat'] . ", sunat_log = '". json_encode($resp) ."' , url_cdr = '". $resp['ruta_cdr'] ."' WHERE id = " . $idDocumento . ";";
					}
					$result = $conn->query($string);
					// echo "Number of rows: $result->num_rows";
					// $result->close();
	...
	function get_array_cabecera($data, $emisor) {
		$cabecera = array(
			...
			'TOTAL_ICBPER' => (isset($data['total_icbper'])) ? $data['total_icbper'] : "0",
